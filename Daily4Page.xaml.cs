...
public partial class Daily4Page : ContentPage
{
    const int Rows = 10;
    const int Cols = 4;
    ...
    internal static string ComingFrom { get; set; } = "d3";

    public Daily4Page()
    {
        InitializeComponent();
        _wLabels = new[] { lblW1, lblW2, lblW3, lblW4 };
        BuildRows();
        BuildSlotPicker();
    ...
    private async void OnPagePan(object? sender, PanUpdatedEventArgs e)
    {
        ...
        switch (e.StatusType)
        {
            ...
            case GestureStatus.Canceled:
                if (_panLeft < -40) // left → go to DD
                {
                    _isPanning = true;
                    DailyDerbyPage.ComingFrom = "d4";
    ...
    {
        if (ComingFrom == "d3")
            AppShell.Daily3PageInstance.PrePosition(false);
        else
        {
    ...
    private async void BtnGameMenu_Clicked(object sender, EventArgs e)
    {
        ...
        if (choice == "Check Wins for Draw#") { DrawSearchPage.PresetGame = "Daily 4"; await Shell.Current.GoToAsync(nameof(DrawSearchPage), false); return; }
    ...
    private async void BtnNumIntel_Clicked(object sender, EventArgs e)
    {
        ...
        if (choice == null || choice == "Cancel") return;
        if (choice == "Back-Test Numbers")    { BackTestPage.PresetGame      = "Daily 4"; await Shell.Current.GoToAsync(nameof(BackTestPage),      false); return; }
        if (choice == "Hot & Cold Numbers")   { HotColdPage.PresetGame       = "Daily 4"; await Shell.Current.GoToAsync(nameof(HotColdPage),       false); return; }
        if (choice == "Positional Frequency") { PositionalFreqPage.PresetGame = "Daily 4"; await Shell.Current.GoToAsync(nameof(PositionalFreqPage), false); return; }
        if (choice == "Sum Range")            { SumRangePage.PresetGame      = "Daily 4"; await Shell.Current.GoToAsync(nameof(SumRangePage),      false); return; }
        if (choice == "Balance Check")        { BalanceCheckPage.PresetGame  = "Daily 4"; await Shell.Current.GoToAsync(nameof(BalanceCheckPage),  false); return; }
        if (choice == "Pairs & Triplets")     { PairsTripletsPage.PresetGame = "Daily 4"; await Shell.Current.GoToAsync(nameof(PairsTripletsPage), false); return; }
        if (choice == "Number Gap Tracker")   { GapTrackerPage.PresetGame    = "Daily 4"; await Shell.Current.GoToAsync(nameof(GapTrackerPage),    false); return; }
        if (choice == "Ticket Scorer")        { TicketScorerPage.PresetGame  = "Daily 4"; await Shell.Current.GoToAsync(nameof(TicketScorerPage),  false); return; }
        if (choice == "Wheeling System")      { WheelingPage.PresetGame      = "Daily 4"; await Shell.Current.GoToAsync(nameof(WheelingPage),      false); return; }
        if (choice == "Rundown Calculator")   { RundownPage.PresetGame       = "Daily 4"; await Shell.Current.GoToAsync(nameof(RundownPage),       false); return; }
    }

    private async void BtnGoDD_Clicked(object sender, EventArgs e)
    {
        if (_isPanning) return;
        DailyDerbyPage.ComingFrom = "d4";
        AppShell.DailyDerbyPageInstance.PrePosition(true);
    ...
    protected override void OnAppearing()
    {
        ...
        {
            int pendingRow = -1;
            if (PendingHighlight.HasPending && PendingHighlight.Game == "D4")
            {
                _activeSlot = PendingHighlight.Slot;
                pendingRow  = PendingHighlight.Row;
                PendingHighlight.Clear();
                Preferences.Set("d4_active_slot", _activeSlot);
                FillFromSlot(_activeSlot);
            }
            else
            {
                _activeSlot = Preferences.Get("d4_active_slot", -1);
                if (_activeSlot < 0)
                {
                    var current = Preferences.Get("d4_entries", "");
    ...
    async Task LogCurrentTicketsAsync()
    {
        ...
        for (int s = 0; s < 10; s++)
        {
            ...
            for (int r = 0; r < Rows; r++)
            {
                ...
                rows.Add((s, r, string.Join(" ", nums), "", pf, pt));
            }
        }
        for (int s = 0; s < 10; s++)
            await Services.TicketLogService.ClearTodayGameSlotAsync("D4", s);
        await Services.TicketLogService.LogRowsAsync("D4", rows);
    }
    ...
    private void SaveEntries()
    {
        ...
        Preferences.Set("d4_entries", string.Join("|", vals));
        Preferences.Set("d4_bettypes", string.Join("|", _betTypes));
        Preferences.Set("d4_from", fromEntry.Text ?? "");
    }
    ...
    private void LoadEntries()
    {
        _loading = true;
        ClearAllEntries();
        var saved = Preferences.Get("d4_entries", "");
        ...
        LoadBetTypes("d4_bettypes");
        fromEntry.Text = Preferences.Get("d4_from", "");
        _loading = false;
    ...
    private string SetKey(int slot) => $"d4_set_{slot}";
    private void UpdateTicketCount()
    {
        ...
        for (int s = 0; s < 10; s++)
        {
            ...
            {
                var vals = Preferences.Get($"d4_set_{s}", "").Split('|');
                for (int r = 0; r < 10; r++)
                    if (r * 4 < vals.Length && !string.IsNullOrWhiteSpace(vals[r * 4])) total++;
            }
        }
        lblTicketCount.Text = total > 0 ? $"🎟 {total}" : "";
    }
    private string BetKey(int slot) => $"d4_btypes_{slot}";
    ...
    private string ExclKey(int slot) => $"excl_set_d4_{slot}";
    ...
    private async void BtnAdvAllCombinedSet_Clicked(object sender, EventArgs e)
    {
        ...
        if (hasDraw)
        {
            string? warn = SpendingTracker.CheckDrawRangeWarning("D4", from, to, ds, de,
                currentDrawNumber: DrawNumberService.GetNextDraw("Daily 4"));
            if (warn != null)
            {
                bool proceed = await DisplayAlert("Check Draw # Range", warn, "Save Anyway", "Fix It");
                if (!proceed)
                {
                    int current = DrawNumberService.GetNextDraw("Daily 4");
                    if (current <= 0) current = await DrawNumberService.EnsureNextDrawAsync("Daily 4");
                    if (current > 0)
                    {
    ...
    private void SlotPicker_Changed(object sender, EventArgs e)
    {
        ...
        SaveAdvanceDates(_activeSlot);
        _activeSlot = slot;
        Preferences.Set("d4_active_slot", slot);
        ClearAllEntries();
        _suppressAdvApply = true;
        try
        {
    ...
    // ── Build 10 input rows (4 boxes each) ───────────────────────────────────
        ...
        for (int r = 0; r < Rows; r++)
        {
            var row = new Grid
            {
                ColumnDefinitions =
                {
                    ...
                    new ColumnDefinition(GridLength.Star),       // 4: digit4
                    new ColumnDefinition(GridLength.Auto),       // 5: perm checkbox
                    new ColumnDefinition(new GridLength(40)),    // 6: bet type
    ...
    static List<string> GetUniquePerms(string[] parts, string sep)
    {
        ...
        PermuteD4(arr, 0, set, sep);
        return set.ToList();
    }

    static void PermuteD4(string[] arr, int start, HashSet<string> results, string sep)
    {
        if (start == arr.Length - 1) { results.Add(string.Join(sep, arr)); return; }
        for (int i = start; i < arr.Length; i++)
        {
            (arr[start], arr[i]) = (arr[i], arr[start]);
            PermuteD4(arr, start + 1, results, sep);
            (arr[start], arr[i]) = (arr[i], arr[start]);
        }
    }
    ...
    private void AllBetPicker_Changed(object sender, EventArgs e)
    {
        ...
        // Apply only to the active set — not every saved set
        if (_activeSlot >= 0)
        {
            SaveSet(_activeSlot);
            _slotCache[_activeSlot] = (GetCurrentEntryString(), string.Join("|", _betTypes));
    ...
    private async Task LoadAllDraws()
    {
        ...
        spinner.IsRunning = true;
        loadingOverlay.IsVisible = true;
        lblDrawDate.Text = "Loading Daily 4 history...";

        var raw = await GetDataEntry.LoadD4CsvDraws();
        ...
        if (raw.Count == 0)
        {
            string errMsg = string.IsNullOrEmpty(GetDataEntry.LastError)
                ? "Daily4: Could not fetch — check internet connection"
                : $"Daily4: {GetDataEntry.LastError}";
    ...
    private void CheckAll()
    {
        for (int r = 0; r < Rows; r++)
        {
            ...
            bool isStr = _winNums != null && userNums.SequenceEqual(_winNums);
            bool isBox = !isStr && _winNums != null &&
                         userNums.OrderBy(x => x).SequenceEqual(_winNums.OrderBy(x => x));

            // Apply bet type filter
            string? win = null;
            if (bt == "S")
            {
                if (isStr) win = "Straight";
            }
            else if (bt == "B")
            {
                if (isStr || isBox) win = "Box";
            }
            else // S&B
            {
                if (isStr)      win = "S&B!";
                else if (isBox) win = "Box";
            }
    ...
    private async void BtnSave_Clicked(object sender, EventArgs e)
    {
        ...
        if (choice == "Save to MyFavorite")
        {
            ...
            await MyFavoritePage.SaveCurrentToMyFavoriteAsync(
                "Daily 4", "d4_set_", _activeSlot < 0 ? 0 : _activeSlot,
    ...
    private void BuildAdvancePlayOverlay()
    {
        ...
        void UpdateAdvWarning()
        {
            string w = SpendingTracker.CheckDrawRangeWarning(
                "D4", _advStartPicker!.Date ?? DateTime.Today, _advEndPicker!.Date ?? DateTime.Today,
        ...
        btnOk.Clicked += async (_, _) =>
        {
            ...
            string? warn = SpendingTracker.CheckDrawRangeWarning("D4", _advStartPicker!.Date ?? DateTime.Today, _advEndPicker!.Date ?? DateTime.Today, ds, de,
                currentDrawNumber: DrawNumberService.GetNextDraw("Daily 4"));
            if (warn != null)
            {
                bool proceed = await DisplayAlert("Check Draw # Range", warn, "Save Anyway", "Fix It");
            ...
            UpdateResultBackground(_advRow);
            SaveAdvanceDates(_activeSlot);
            _ = TicketLogService.RecordAdvanceEnteredAsync("D4", _activeSlot, _advRow, _playStart[_advRow]!.Value, _playEnd[_advRow]!.Value);
    ...
    private string AdvDatesKey(int slot) => $"d4_adv_{slot}";
    ...
    private async void RefreshAdvAllPanel()
    {
        ...
        int next = await DrawNumberService.EnsureNextDrawAsync("Daily 4");
