...
public partial class DailyDerbyPage : ContentPage
{
    ...
    private async void BtnGameMenu_Clicked(object sender, EventArgs e)
    {
        ...
        if (choice == "Check Wins for Draw#") { DrawSearchPage.PresetGame = "Daily Derby"; await Shell.Current.GoToAsync(nameof(DrawSearchPage), false); return; }
    ...
    private async void BtnNumIntel_Clicked(object sender, EventArgs e)
    {
        string? choice = await DisplayActionSheet("Number Intelligence", "Cancel", null,
            ...
            "Number Gap Tracker", "Ticket Scorer", "Wheeling System");
        if (choice == null || choice == "Cancel") return;
        if (choice == "Back-Test Numbers")    { BackTestPage.PresetGame      = "Daily Derby"; await Shell.Current.GoToAsync(nameof(BackTestPage),      false); return; }
        if (choice == "Hot & Cold Numbers")   { HotColdPage.PresetGame       = "Daily Derby"; await Shell.Current.GoToAsync(nameof(HotColdPage),       false); return; }
        if (choice == "Positional Frequency") { PositionalFreqPage.PresetGame = "Daily Derby"; await Shell.Current.GoToAsync(nameof(PositionalFreqPage), false); return; }
        if (choice == "Sum Range")            { SumRangePage.PresetGame      = "Daily Derby"; await Shell.Current.GoToAsync(nameof(SumRangePage),      false); return; }
        if (choice == "Balance Check")        { BalanceCheckPage.PresetGame  = "Daily Derby"; await Shell.Current.GoToAsync(nameof(BalanceCheckPage),  false); return; }
        if (choice == "Pairs & Triplets")     { PairsTripletsPage.PresetGame = "Daily Derby"; await Shell.Current.GoToAsync(nameof(PairsTripletsPage), false); return; }
        if (choice == "Number Gap Tracker")   { GapTrackerPage.PresetGame    = "Daily Derby"; await Shell.Current.GoToAsync(nameof(GapTrackerPage),    false); return; }
        if (choice == "Ticket Scorer")        { TicketScorerPage.PresetGame  = "Daily Derby"; await Shell.Current.GoToAsync(nameof(TicketScorerPage),  false); return; }
        if (choice == "Wheeling System")      { WheelingPage.PresetGame      = "Daily Derby"; await Shell.Current.GoToAsync(nameof(WheelingPage),      false); return; }
    }
    ...
    void TakeLogSnapshot()
    {
        for (int r = 0; r < Rows; r++)
        {
            ...
            {
                string v = _horseEntries[r, c].Text ?? "";
    ...
    async Task LogCurrentTicketsAsync()
    {
        ...
        for (int s = 0; s < 10; s++)
        {
            ...
            for (int r = 0; r < Rows; r++)
            {
                ...
                for (int c = 0; c < HorseCols; c++)
                {
                    int storedCols = vals.Length <= Rows * HorseCols ? HorseCols : 4;
                    string v = r * storedCols + c < vals.Length ? vals[r * storedCols + c] : "";
                    if (string.IsNullOrWhiteSpace(v)) { full = false; break; }
                    nums.Add(v);
                }
                if (!full) continue;
        ...
        for (int s = 0; s < 10; s++)
            await Services.TicketLogService.ClearTodayGameSlotAsync("DD", s);
        await Services.TicketLogService.LogRowsAsync("DD", rows);
    }
    ...
    private void UpdateTicketCount()
    {
        ...
        for (int s = 0; s < 10; s++)
        {
            ...
            {
                for (int r = 0; r < Rows; r++)
                    if (!string.IsNullOrWhiteSpace(_horseEntries[r, 0]?.Text)) total++;
            }
            else
            {
                var vals = Preferences.Get($"dd_set_{s}", "").Split('|');
                for (int r = 0; r < 10; r++)
    ...
    private async void BtnAdvAllCombinedSet_Clicked(object sender, EventArgs e)
    {
        ...
        if (hasDraw)
        {
            string? warn = SpendingTracker.CheckDrawRangeWarning("DD", from, to, ds, de,
                currentDrawNumber: DrawNumberService.GetNextDraw("Daily Derby"));
            if (warn != null)
            {
                bool proceed = await DisplayAlert("Check Draw # Range", warn, "Save Anyway", "Fix It");
                if (!proceed)
                {
                    int current = DrawNumberService.GetNextDraw("Daily Derby");
                    if (current <= 0) current = await DrawNumberService.EnsureNextDrawAsync("Daily Derby");
                    if (current > 0)
                    {
        ...
        if (Preferences.Get("set_row_picker", true))
        {
            ...
            if (choice == "Clear All Advances")
            {
                ...
                {
                    for (int c = 0; c < HorseCols; c++)
                        if (string.IsNullOrWhiteSpace(_horseEntries[r, c].Text))
                            _horseEntries[r, c].BackgroundColor = Color.FromArgb("#F5F5F5");
                    if (string.IsNullOrWhiteSpace(_timeEntries[r].Text))
        ...
        // Clear all existing advance dates first, then apply to first N rows
        for (int r = 0; r < Rows; r++)
        {
        ...
        for (int r = 0; r < Rows; r++)
        {
            ...
            for (int c = 0; c < HorseCols; c++)
                if (string.IsNullOrWhiteSpace(_horseEntries[r, c].Text))
                    _horseEntries[r, c].BackgroundColor = bg;
            if (string.IsNullOrWhiteSpace(_timeEntries[r].Text))
                _timeEntries[r].BackgroundColor = bg;
        }
    }
    ...
    private void BuildRows()
    {
        for (int r = 0; r < Rows; r++)
        {
            ...
            for (int c = 0; c < HorseCols; c++)
            {
                ...
                int row_ = r, col_ = c;
                entry.Focused += (_, _) => { if (!_loading && !_retreating) Dispatcher.Dispatch(() => _horseEntries[row_, col_].Text = ""); _retreating = false; };
                EntryHelper.AttachBackspace(entry, () => RetreatHorseFocus(row_, col_));
                entry.TextChanged += (s, e) =>
                {
                    ...
                    {
                        for (int ci = 0; ci < HorseCols; ci++)
                        {
                            if (ci == col_) continue;
                            if (int.TryParse(_horseEntries[row_, ci].Text, out int ex) && ex == entered)
                            {
    ...
    private void RetreatHorseFocus(int row, int col)
    {
        _retreating = true;
        if (col > 0) EntryHelper.SelectAll(_horseEntries[row, col - 1]);
        else if (row > 0) EntryHelper.SelectAll(_timeEntries[row - 1]);
    }
    ...
    private void BuildAdvancePlayOverlay()
    {
        ...
        void UpdateAdvWarning()
        {
            string w = SpendingTracker.CheckDrawRangeWarning(
                "DD", _advStartPicker!.Date ?? DateTime.Today, _advEndPicker!.Date ?? DateTime.Today,
        ...
        btnOk.Clicked += async (_, _) =>
        {
            ...
            string? warn = SpendingTracker.CheckDrawRangeWarning("DD", _advStartPicker!.Date ?? DateTime.Today, _advEndPicker!.Date ?? DateTime.Today, ds, de,
                currentDrawNumber: DrawNumberService.GetNextDraw("Daily Derby"));
            if (warn != null)
            {
                bool proceed = await DisplayAlert("Check Draw # Range", warn, "Save Anyway", "Fix It");
            ...
            UpdateResultBackground(_advRow);
            SaveAdvanceDates(_activeSlot);
            _ = TicketLogService.RecordAdvanceEnteredAsync("DD", _activeSlot, _advRow, _playStart[_advRow]!.Value, _playEnd[_advRow]!.Value);
    ...
    private async void RefreshAdvAllPanel()
    {
        ...
        int next = await DrawNumberService.EnsureNextDrawAsync("Daily Derby");
