...
public partial class SuperLottoPage : ContentPage
{
    ...
    private async void BtnGameMenu_Clicked(object sender, EventArgs e)
    {
        ...
        if (choice == "Check Wins for Draw#") { DrawSearchPage.PresetGame = "Super Lotto"; await Shell.Current.GoToAsync(nameof(DrawSearchPage), false); return; }
    ...
    private async void BtnNumIntel_Clicked(object sender, EventArgs e)
    {
        ...
        if (choice == null || choice == "Cancel") return;
        if (choice == "Back-Test Numbers")    { BackTestPage.PresetGame      = "Super Lotto"; await Shell.Current.GoToAsync(nameof(BackTestPage),      false); return; }
        if (choice == "Hot & Cold Numbers")   { HotColdPage.PresetGame       = "Super Lotto"; await Shell.Current.GoToAsync(nameof(HotColdPage),       false); return; }
        if (choice == "Positional Frequency") { PositionalFreqPage.PresetGame = "Super Lotto"; await Shell.Current.GoToAsync(nameof(PositionalFreqPage), false); return; }
        if (choice == "Sum Range")            { SumRangePage.PresetGame      = "Super Lotto"; await Shell.Current.GoToAsync(nameof(SumRangePage),      false); return; }
        if (choice == "Balance Check")        { BalanceCheckPage.PresetGame  = "Super Lotto"; await Shell.Current.GoToAsync(nameof(BalanceCheckPage),  false); return; }
        if (choice == "Pairs & Triplets")     { PairsTripletsPage.PresetGame = "Super Lotto"; await Shell.Current.GoToAsync(nameof(PairsTripletsPage), false); return; }
        if (choice == "Number Gap Tracker")   { GapTrackerPage.PresetGame    = "Super Lotto"; await Shell.Current.GoToAsync(nameof(GapTrackerPage),    false); return; }
        if (choice == "Ticket Scorer")        { TicketScorerPage.PresetGame  = "Super Lotto"; await Shell.Current.GoToAsync(nameof(TicketScorerPage),  false); return; }
        if (choice == "Wheeling System")      { WheelingPage.PresetGame      = "Super Lotto"; await Shell.Current.GoToAsync(nameof(WheelingPage),      false); return; }
        if (choice == "Combo Filter")         { ComboFilterPage.PresetGame   = "Super Lotto"; await Shell.Current.GoToAsync(nameof(ComboFilterPage),   false); return; }
    }
    ...
    protected override void OnAppearing()
    {
        ...
        {
            int pendingRow = -1;
            if (PendingHighlight.HasPending && PendingHighlight.Game == "SL")
            {
    ...
    void TakeLogSnapshot()
    {
        for (int r = 0; r < Rows; r++)
        {
            ...
            string mega = _entries[r, MegaCol].Text ?? "";
            _logSnapshot[r] = (full && !string.IsNullOrWhiteSpace(mega))
                ? string.Join(" ", nums) + " " + mega.PadLeft(2, '0')
                : "";
        }
    }
    ...
    {
        var rows = new List<(int, int, string, string, string, string, int)>();
        for (int s = 0; s < 10; s++)
        {
            ...
            for (int r = 0; r < Rows; r++)
            {
                ...
                for (int c = 0; c < MainCols; c++)
                {
                    string v = r * TotalCols + c < vals.Length ? vals[r * TotalCols + c] : "";
                    if (string.IsNullOrWhiteSpace(v)) { full = false; break; }
                    nums.Add(v.PadLeft(2, '0'));
                }
                string mega = r * TotalCols + MegaCol < vals.Length ? vals[r * TotalCols + MegaCol] : "";
                if (!full || string.IsNullOrWhiteSpace(mega)) continue;
                string pf = "", pt = "";
                int dc = 0;
                if (!string.IsNullOrEmpty(advParts[r]))
                {
                    ...
                    if (pair.Length >= 4 && int.TryParse(pair[2], out int ds) && int.TryParse(pair[3], out int de) && ds > 0 && de >= ds)
                        dc = Math.Abs(de - ds) + 1;
                }
                rows.Add((s, r, string.Join(" ", nums), $"M:{mega.PadLeft(2,'0')}", pf, pt, dc));
            }
        }
        for (int s = 0; s < 10; s++)
            await Services.TicketLogService.ClearTodayGameSlotAsync("SL", s);
        await Services.TicketLogService.LogRowsWithDrawCountAsync("SL", rows);
    }
    ...
    private void UpdateTicketCount()
    {
        ...
        for (int s = 0; s < 10; s++)
        {
            ...
            {
                var vals = Preferences.Get($"sl_set_{s}", "").Split('|');
                for (int r = 0; r < 10; r++)
                    if (r * 6 < vals.Length && !string.IsNullOrWhiteSpace(vals[r * 6])) total++;
            }
        }
    ...
    private void ClearAllEntries()
    {
        ...
        {
            for (int c = 0; c < MainCols; c++)
                { _entries[r, c].Text = ""; _entries[r, c].BackgroundColor = Colors.White; }
            _entries[r, MegaCol].Text = "";
            _entries[r, MegaCol].BackgroundColor = Colors.White;
        }
    ...
    private void ClearRow(int r)
    {
        ...
        {
            _entries[r, c].Text = "";
            _entries[r, c].BackgroundColor = Colors.White;
        }
        _entries[r, MegaCol].Text = "";
        _entries[r, MegaCol].BackgroundColor = Colors.White;
        _results[r].Text = "";
    ...
    private async void BtnAdvAllCombinedSet_Clicked(object sender, EventArgs e)
    {
        ...
        if (hasDraw)
        {
            string? warn = SpendingTracker.CheckDrawRangeWarning("SL", from, to, ds, de,
                currentDrawNumber: DrawNumberService.GetNextDraw("Super Lotto"));
            if (warn != null)
            {
                bool proceed = await DisplayAlert("Check Draw # Range", warn, "Save Anyway", "Fix It");
                if (!proceed)
                {
                    int current = DrawNumberService.GetNextDraw("Super Lotto");
                    if (current <= 0) current = await DrawNumberService.EnsureNextDrawAsync("Super Lotto");
                    if (current > 0)
                    {
        ...
        for (int r = 0; r < Rows; r++)
        {
            ...
            var bg = hasAdv ? Color.FromArgb("#E3F2FD") : Colors.White;
            for (int c = 0; c < MainCols + 1; c++)
                if (string.IsNullOrWhiteSpace(_entries[r, c].Text))
    ...
    private string ExclKey(int slot) => $"excl_set_sl_{slot}";
    ...
    private void BuildRows()
    {
        for (int r = 0; r < Rows; r++)
        {
            ...
            for (int c = 0; c < MainCols; c++)
            {
                var entry = MakeEntry(Colors.White);
                ...
                entry.TextChanged += (s, e) =>
                {
                    ...
                    {
                        if (int.TryParse(nv, out int cv) && cv >= 1) AdvanceFocus(row_, col_);
                        else ((Entry)s!).Text = "";
                    }
                    SaveEntries();
            ...
            var megaEntry = MakeEntry(Colors.White);
            AttachMaxClamp(megaEntry, 27);
            int mrow = r;
            EntryHelper.AttachBackspace(megaEntry, () => RetreatFocus(mrow, MegaCol));
            megaEntry.TextChanged += (_, _) =>
            {
                string megaTxt = _entries[mrow, MegaCol].Text ?? "";
                if (!_voiceSettingText && megaTxt.Length == 2)
                {
                    if (int.TryParse(megaTxt, out int cv) && cv >= 1) AdvanceFocus(mrow, MegaCol);
                    else _entries[mrow, MegaCol].Text = "";
                }
                SaveEntries();
                TriggerAutoSaveIndicator();
    ...
    private async Task LoadAllDraws()
    {
        ...
        // Load CSV history + API prize data in parallel
        var csvTask = GetDataEntry.LoadSLCsvDraws();
        var apiTask = GetDataEntry.GetSuperLottoDraws(30);
        await Task.WhenAll(csvTask, apiTask);
        var raw = csvTask.Result;

        // Build prize lookup by date from API result
        var prizesByDate = new Dictionary<DateTime, DrawPrizeTier[]>();
        foreach (var d in apiTask.Result)
            if (DateTime.TryParse(d.DrawDate, out var dt) && d.Prizes.Length > 0)
                prizesByDate[dt.Date] = d.Prizes;
        ...
        _allDraws = raw
            .Select(d =>
            {
                ...
                return (Date: date, Label: d.DrawDate, DrawNumber: d.DrawNumber, MainNumbers: d.MainNumbers, MegaNumber: d.Mega, Prizes: prizes);
            })
    ...
    static int GetSLTier(int mainMatches, bool megaMatch) => (mainMatches, megaMatch) switch
    {
        (5, true)  => 1,
        (5, false) => 2,
        (4, true)  => 3,
    ...
    string GetSLPrizeLabel(int mainMatches, bool megaMatch)
    {
        int tier = GetSLTier(mainMatches, megaMatch);
        if (tier == 0) return "";
        var pt = _currentPrizeTiers.FirstOrDefault(p => p.Tier == tier);
        if (pt == default) return megaMatch ? $"{mainMatches}+M" : $"{mainMatches}/5";
        decimal amt = pt.Amount;
        if (amt >= 1_000_000) return $"${amt / 1_000_000:0.#}M";
        if (amt >= 1_000)     return $"${amt / 1_000:0.#}K";
        return $"${amt:0}";
    }
        ...
        for (int r = 0; r < Rows; r++)
        {
            ...
            for (int c = 0; c < MainCols; c++)
            {
                ...
                else
                {
                    ...
                    _entries[r, c].BackgroundColor = hasVal ? Color.FromArgb("#FFCDD2") : Colors.White;
                }
            }
            ...
            else
            {
                ...
                _entries[r, MegaCol].BackgroundColor = hasVal ? Color.FromArgb("#FFCDD2") : Colors.White;
            }

            bool hasPrize = megaMatch || mainMatches >= 3;
            string prizeLabel = GetSLPrizeLabel(mainMatches, megaMatch);
            if (mainMatches > 0 || megaMatch)
            {
                _results[r].Text = string.IsNullOrEmpty(prizeLabel)
                    ? (megaMatch ? $"{mainMatches}+M" : $"{mainMatches}/5")
                    : prizeLabel;
    ...
    internal void InvalidateAfterPurge() { _slotCache.Clear(); _advDatesLoaded = false; }
    ...
    private async void BtnQuickPick_Clicked(object sender, EventArgs e)
    {
        ...
        for (int r = 0; r < Rows && filled < max; r++)
        {
            ...
            var main = Enumerable.Range(1, 47).OrderBy(_ => rng.Next()).Take(MainCols).OrderBy(n => n).ToList();
            for (int c = 0; c < MainCols; c++)
                _entries[r, c].Text = main[c].ToString();
            _entries[r, MegaCol].Text = rng.Next(1, 28).ToString();
            filled++;
        }
    ...
    void OnVoiceNumbers(List<int> nums)
    {
        ...
        foreach (int n in nums)
        {
            ...
            int maxForCol = _voiceCol < MainCols ? 47 : 27;
            if (n >= 1 && n <= maxForCol)
            {
    ...
    private async void BtnSave_Clicked(object sender, EventArgs e)
    {
        ...
        if (choice == "Save to MyFavorite")
        {
            ...
            await MyFavoritePage.SaveCurrentToMyFavoriteAsync(
                "SuperLotto Plus", "sl_set_", _activeSlot < 0 ? 0 : _activeSlot, GetCurrentEntryString());
            return;
        }
    ...
    private void BuildAdvancePlayOverlay()
    {
        ...
        void UpdateAdvWarning()
        {
            string w = SpendingTracker.CheckDrawRangeWarning(
                "SL", _advStartPicker!.Date ?? DateTime.Today, _advEndPicker!.Date ?? DateTime.Today,
        ...
        btnOk.Clicked += async (_, _) =>
        {
            ...
            string? warn = SpendingTracker.CheckDrawRangeWarning("SL", _advStartPicker!.Date ?? DateTime.Today, _advEndPicker!.Date ?? DateTime.Today, ds, de,
                currentDrawNumber: DrawNumberService.GetNextDraw("Super Lotto"));
            if (warn != null)
            {
                bool proceed = await DisplayAlert("Check Draw # Range", warn, "Save Anyway", "Fix It");
            ...
            UpdateResultBackground(_advRow);
            SaveAdvanceDates(_activeSlot);
            _ = TicketLogService.RecordAdvanceEnteredAsync("SL", _activeSlot, _advRow, _playStart[_advRow]!.Value, _playEnd[_advRow]!.Value);
    ...
    private string AdvDatesKey(int slot) => $"sl_adv_{slot}";
    ...
    private async void RefreshAdvAllPanel()
    {
        ...
        int next = await DrawNumberService.EnsureNextDrawAsync("Super Lotto");
