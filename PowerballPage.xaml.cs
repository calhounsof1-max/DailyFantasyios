...
public partial class PowerballPage : ContentPage
{
    ...
    private async void BtnGameMenu_Clicked(object sender, EventArgs e)
    {
        ...
        if (choice == "Check Wins for Draw#") { DrawSearchPage.PresetGame = "Powerball"; await Shell.Current.GoToAsync(nameof(DrawSearchPage), false); return; }
    ...
    void TakeLogSnapshot()
    {
        for (int r = 0; r < Rows; r++)
        {
            ...
            string pb = _entries[r, PBCol].Text ?? "";
            _logSnapshot[r] = (full && !string.IsNullOrWhiteSpace(pb))
                ? string.Join(" ", nums) + " " + pb.PadLeft(2, '0')
                : "";
        }
    }
    ...
    async Task LogCurrentTicketsAsync()
    {
        ...
        for (int s = 0; s < 10; s++)
        {
            ...
            for (int r = 0; r < Rows; r++)
            {
                ...
                string pb = r * TotalCols + PBCol < vals.Length ? vals[r * TotalCols + PBCol] : "";
                if (!full || string.IsNullOrWhiteSpace(pb)) continue;
                string pf = "", pt = "";
                int dc = 0;
                ...
                rows.Add((s, r, string.Join(" ", nums), $"PB:{pb.PadLeft(2,'0')}", pf, pt, dc));
            }
        }
        for (int s = 0; s < 10; s++)
            await Services.TicketLogService.ClearTodayGameSlotAsync("PB", s);
        await Services.TicketLogService.LogRowsWithDrawCountAsync("PB", rows);
    }
    ...
    private void UpdateTicketCount()
    {
        ...
        for (int s = 0; s < 10; s++)
        {
            ...
            {
                var vals = Preferences.Get($"pb_set_{s}", "").Split('|');
                for (int r = 0; r < 10; r++)
    ...
    private void ClearAllEntries()
    {
        for (int r = 0; r < Rows; r++)
        {
            ...
            _entries[r, PBCol].BackgroundColor = Colors.White;
        }
    ...
    private async void BtnAdvAllCombinedSet_Clicked(object sender, EventArgs e)
    {
        ...
        if (hasDraw)
        {
            string? warn = SpendingTracker.CheckDrawRangeWarning("PB", from, to, ds, de,
                currentDrawNumber: DrawNumberService.GetNextDraw("Powerball"));
            if (warn != null)
            {
                bool proceed = await DisplayAlert("Check Draw # Range", warn, "Save Anyway", "Fix It");
                if (!proceed)
                {
                    int current = DrawNumberService.GetNextDraw("Powerball");
                    if (current <= 0) current = await DrawNumberService.EnsureNextDrawAsync("Powerball");
                    if (current > 0)
                    {
    ...
    // Confirmed live 2026-08-10 (same root cause found on the Hot Spot page): Android never
    // raises SelectedIndexChanged at all when you re-pick the SAME item that's already
    // selected — the index genuinely didn't move. That left the Draw# "Start #"/"End (opt)"
    // auto-fill (RefreshAdvAllPanel) stuck showing stale/blank values after re-tapping an
    // already-active Set, which had been misdiagnosed in earlier sessions as an intermittent
    // network-fetch failure. Unfocused fires every time the dropdown closes, changed or not,
    // so it's the reliable place to force that specific auto-fill to re-run when nothing else
    // needed to change. Guarded by _suppressPickerEvent so UpdateSlotPicker()'s own
    // programmatic slotPicker.Unfocus() call (used after relabeling items) doesn't also
    // trigger an extra unnecessary fetch.
    private void SlotPicker_Unfocused(object sender, FocusEventArgs e)
    {
        if (!_suppressPickerEvent && slotPicker.SelectedIndex == _activeSlot)
            RefreshAdvAllPanel();
    }
    ...
    private void BuildRows()
    {
        for (int r = 0; r < Rows; r++)
        {
            ...
            // Powerball entry
            var pbEntry = MakeEntry(Colors.White);
            AttachMaxClamp(pbEntry, 26);
            int mrow = r;
            EntryHelper.AttachBackspace(pbEntry, () => RetreatFocus(mrow, PBCol));
            pbEntry.TextChanged += (_, _) =>
            {
                string pbTxt = _entries[mrow, PBCol].Text ?? "";
                if (!_voiceSettingText && pbTxt.Length == 2)
                {
                    if (int.TryParse(pbTxt, out int cv) && cv >= 1) AdvanceFocus(mrow, PBCol);
                    else _entries[mrow, PBCol].Text = "";
                }
                SaveEntries();
                TriggerAutoSaveIndicator();
    ...
    private async Task LoadAllDraws(bool forceApi = false)
    {
        ...
        spinner.IsRunning = true;
        loadingOverlay.IsVisible = true;
        lblDrawDate.Text = "Loading Powerball history...";

        List<(string DrawDate, int DrawNumber, int[] MainNumbers, int PBNumber, DrawPrizeTier[] Prizes)> raw;

        if (forceApi)
        {
            lblDrawDate.Text = "Fetching draws from calottery.com...";
            await GetDataEntry.UpdatePBCsvAsync();
            raw = await GetDataEntry.LoadPBCsvDraws();
        }
        else
        {
            raw = await GetDataEntry.LoadPBCsvDraws();
        ...
        if (raw.Count == 0)
        {
            string errMsg = string.IsNullOrEmpty(GetDataEntry.LastError)
                ? "Powerball: Could not load draw history"
    ...
    private void CheckAll()
    {
        ...
        for (int r = 0; r < Rows; r++)
        {
            ...
            else
            {
                ...
                _entries[r, PBCol].BackgroundColor = hasVal ? Color.FromArgb("#FFCDD2") : Colors.White;
            }
    ...
    private void BuildAdvancePlayOverlay()
    {
        ...
        void UpdateAdvWarning()
        {
            string w = SpendingTracker.CheckDrawRangeWarning(
                "PB", _advStartPicker!.Date ?? DateTime.Today, _advEndPicker!.Date ?? DateTime.Today,
        ...
        btnOk.Clicked += async (_, _) =>
        {
            ...
            string? warn = SpendingTracker.CheckDrawRangeWarning("PB", _advStartPicker!.Date ?? DateTime.Today, _advEndPicker!.Date ?? DateTime.Today, ds, de,
                currentDrawNumber: DrawNumberService.GetNextDraw("Powerball"));
            if (warn != null)
            {
                bool proceed = await DisplayAlert("Check Draw # Range", warn, "Save Anyway", "Fix It");
            ...
            UpdateResultBackground(_advRow);
            SaveAdvanceDates(_activeSlot);
            _ = TicketLogService.RecordAdvanceEnteredAsync("PB", _activeSlot, _advRow, _playStart[_advRow]!.Value, _playEnd[_advRow]!.Value);
    ...
    private async void RefreshAdvAllPanel()
    {
        ...
        int next = await DrawNumberService.EnsureNextDrawAsync("Powerball");
